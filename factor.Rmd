...
Sometimes we might want to determine the values of $F$ for each subject because we want to know the influence they have on $X$. But $F$ for a given subject is not an knowns parameter, its an unobservable random vector hence it can't be estimated in the usual sense.
...
$X$ is obserbedd, $F$ is not so 'best predictor' would be conditional on expectation $E(F|X)$. Need joint distribution to do that.

Assume the random vector of $X$ and $F$ are MVN distribution with mean vector $\mu$ and 0 respectively. To get covariance matrix:

We know $\operatorname{cov}(x) = \mathbb{L}\mathbb{L}^\prime + \Psi$ and
...
test_sesame <- sesame %>% 
  ...
  select(-pretest) # don't want pretest
